...
<style>
    ...
    .stApp {
        ...
        background-image: 
            ...
            radial-gradient(ellipse at 80% 80%, rgba(240, 147, 251, 0.08) 0%, transparent 50%);
    }
...
def show_important_dates_legend():
    if IMPORTANT_DATES:
        ...
        legend_html += '<span style="color: rgba(255,255,255,0.3); font-size: 0.7rem; letter-spacing: 1.5px; text-transform: uppercase;">📅 Важные даты</span>'
        for date_str, props in IMPORTANT_DATES.items():
            date_obj = pd.to_datetime(date_str)
            legend_html += f'<span class="date-badge">'
            legend_html += f'<span style="color: {props["color"]}; font-size: 1rem;">●</span>'
            legend_html += f'<span style="color: rgba(255,255,255,0.7);">{props["label"]}</span>'
            legend_html += f'<span style="color: rgba(255,255,255,0.2); font-size: 0.65rem;">{date_obj.strftime("%d.%m.%Y")}</span>'
            legend_html += '</span>'
        legend_html += '</div>'
...
def get_pdf_download_link(df, filename="report.pdf"):
    try:
        ...
        href = f'<a href="data:application/pdf;base64,{b64}" download="{filename}" style="display: inline-block; background: rgba(255,255,255,0.05); backdrop-filter: blur(10px); border: 1px solid rgba(255,255,255,0.1); color: white; padding: 12px 30px; border-radius: 10px; text-decoration: none; font-weight: 500; font-size: 1rem; transition: all 0.3s ease;">📥 Скачать отчет (PDF)</a>'
        return href
        ...
        try:
            ...
            return f'<a href="data:file/csv;base64,{b64}" download="{filename}" style="display: inline-block; background: rgba(255,255,255,0.05); backdrop-filter: blur(10px); border: 1px solid rgba(255,255,255,0.1); color: white; padding: 12px 30px; border-radius: 10px; text-decoration: none; font-weight: 500; font-size: 1rem;">📥 Скачать отчет (CSV)</a>'
        except:
...
with st.sidebar:
    st.markdown("---")
    st.markdown('<span style="color: rgba(255,255,255,0.2); font-size: 0.65rem; letter-spacing: 2px; text-transform: uppercase;">📅 Важные даты</span>', unsafe_allow_html=True)
    for date_str, props in IMPORTANT_DATES.items():
        date_obj = pd.to_datetime(date_str)
        st.markdown(f'<div style="display: flex; align-items: center; gap: 0.5rem; padding: 0.15rem 0;"><span style="color: {props["color"]}; font-size: 1rem;">●</span><span style="color: rgba(255,255,255,0.5); font-size: 0.75rem;">{props["label"]}</span><span style="color: rgba(255,255,255,0.15); font-size: 0.65rem;">{date_obj.strftime("%d.%m.%Y")}</span></div>', unsafe_allow_html=True)
    st.markdown("---")
...
if page == "📊 Общая аналитика":
    with st.sidebar:
        st.markdown('<span style="color: rgba(255,255,255,0.2); font-size: 0.65rem; letter-spacing: 2px; text-transform: uppercase;">🎯 Фильтры</span>', unsafe_allow_html=True)
        st.markdown("---")
    ...
    <div class="info-block">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 1rem;">
            <div><div style="color: #4facfe; font-weight: 600; font-size: 0.95rem; margin-bottom: 0.2rem;">🎬 Старты</div><div style="color: rgba(255,255,255,0.4); font-size: 0.8rem;">Количество запусков выпуска</div></div>
            <div><div style="color: #f5576c; font-weight: 600; font-size: 0.95rem; margin-bottom: 0.2rem;">🎧 Стримы</div><div style="color: rgba(255,255,255,0.4); font-size: 0.8rem;">Прослушивания > 2 минут</div></div>
            <div><div style="color: #43e97b; font-weight: 600; font-size: 0.95rem; margin-bottom: 0.2rem;">📈 Конверсия</div><div style="color: rgba(255,255,255,0.4); font-size: 0.8rem;">Стримы / Старты × 100%</div></div>
            <div style="grid-column: span 3; margin-top: 0.3rem; padding-top: 0.6rem; border-top: 1px solid rgba(255,255,255,0.03);">
                <div style="color: #f093fb; font-weight: 600; font-size: 0.95rem; margin-bottom: 0.2rem;">⭐ RSI — Индекс успешности выпуска</div>
                <div style="color: rgba(255,255,255,0.4); font-size: 0.8rem;"><strong style="color: #f5576c;">Стримы</strong> × (<strong style="color: #43e97b;">Конверсия</strong> + 1) × <strong style="color: #4facfe;">Старты<sup>0.1</sup></strong></div>
            </div>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    fig1.add_trace(go.Scatter(x=daily_stats['Дата прослушивания'], y=daily_stats['Старты'], name='Старты', line=dict(color='#4facfe', width=3), fill='tozeroy', fillcolor='rgba(79, 172, 254, 0.08)', mode='lines+markers', marker=dict(size=5, color='white', line=dict(color='#4facfe', width=1.5))))
    fig1.add_trace(go.Scatter(x=daily_stats['Дата прослушивания'], y=daily_stats['Стримы'], name='Стримы', line=dict(color='#f5576c', width=3), fill='tozeroy', fillcolor='rgba(245, 87, 108, 0.08)', mode='lines+markers', marker=dict(size=5, color='white', line=dict(color='#f5576c', width=1.5))))
    fig1.add_trace(go.Scatter(x=daily_stats['Дата прослушивания'], y=daily_stats['Конверсия'], name='Конверсия (%)', line=dict(color='#43e97b', width=2, dash='dash'), mode='lines+markers', marker=dict(size=4, color='#43e97b')), secondary_y=True)
    ...
    fig1.update_layout(
        ...
        height=420,
        hovermode='x unified',
        legend=dict(orientation='h', yanchor='bottom', y=1.02, xanchor='right', x=1, font=dict(color='rgba(255,255,255,0.5)', size=11)),
        xaxis=dict(title='Дата', titlefont=dict(color='rgba(255,255,255,0.3)'), tickfont=dict(color='rgba(255,255,255,0.3)'), gridcolor='rgba(255,255,255,0.03)'),
        yaxis=dict(title='Количество', titlefont=dict(color='rgba(255,255,255,0.3)'), tickfont=dict(color='rgba(255,255,255,0.3)'), gridcolor='rgba(255,255,255,0.03)'),
    ...
    show_hint("💡", "Как читать", "Каждый кружок — выпуск. Чем правее — тем популярнее. Чем выше — тем качественнее. Зеленые кружки — хиты.")
    ...
    if not heatmap_data.empty:
        ...
        fig_heatmap.add_trace(go.Scatter(
            ...
            textfont=dict(color='rgba(255,255,255,0.5)', size=8),
            marker=dict(
                size=heatmap_data['Средний_прослушивания'] * 20 + 8,
                color=heatmap_data['RSI'], colorscale='Viridis', showscale=True,
                colorbar=dict(title='RSI', titlefont=dict(color='rgba(255,255,255,0.3)'), tickfont=dict(color='rgba(255,255,255,0.3)')),
                line=dict(color='rgba(255,255,255,0.1)', width=1), sizemode='area',
                sizeref=2.*max(heatmap_data['Средний_прослушивания'] * 20 + 8)/(40.**2), sizemin=4
            ),
        ...
        fig_heatmap.add_annotation(x=max_x * 0.85, y=max_y * 0.85, text="⭐ ХИТЫ", showarrow=False, font=dict(color='#43e97b', size=14, weight=700), opacity=0.3)
        fig_heatmap.add_annotation(x=max_x * 0.15, y=max_y * 0.85, text="💎 НИШЕВЫЕ", showarrow=False, font=dict(color='#4facfe', size=14, weight=700), opacity=0.3)
        fig_heatmap.add_annotation(x=max_x * 0.85, y=max_y * 0.15, text="📊 МАССОВЫЕ", showarrow=False, font=dict(color='#f6d365', size=14, weight=700), opacity=0.3)
        fig_heatmap.add_annotation(x=max_x * 0.15, y=max_y * 0.15, text="❌ ПРОВАЛЫ", showarrow=False, font=dict(color='#f5576c', size=14, weight=700), opacity=0.3)
        ...
        fig_heatmap.update_layout(
            ...
            height=480,
            hovermode='closest',
            xaxis=dict(title='Старты (популярность)', titlefont=dict(color='rgba(255,255,255,0.3)'), tickfont=dict(color='rgba(255,255,255,0.3)'), gridcolor='rgba(255,255,255,0.03)', type='log'),
            yaxis=dict(title='Дослушиваемость % (качество)', titlefont=dict(color='rgba(255,255,255,0.3)'), tickfont=dict(color='rgba(255,255,255,0.3)'), gridcolor='rgba(255,255,255,0.03)', range=[0, 105]),
            legend=dict(orientation='h', yanchor='bottom', y=1.02, xanchor='right', x=1, font=dict(color='rgba(255,255,255,0.3)'))
        )
    ...
    if not hall_data.empty:
        ...
        with col1:
            st.markdown("""<div style="margin-bottom: 0.8rem;"><span style="font-size: 1.3rem; font-weight: 600; color: #43e97b;">🏆 Зал славы</span><span style="font-size: 0.65rem; color: rgba(255,255,255,0.15); margin-left: 0.5rem; letter-spacing: 1.5px; text-transform: uppercase;">лучшая дослушиваемость</span></div>""", unsafe_allow_html=True)
            for i, (_, row) in enumerate(hall_of_fame.iterrows()):
                medal = ['🥇', '🥈', '🥉', '4️⃣', '5️⃣'][i]
                st.markdown(f"""
                <div class="hall-of-fame">
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        <div>
                            ...
                            <span style="color: white; font-weight: 500; margin-left: 0.4rem; font-size: 0.9rem;">{row['Короткое название']}</span>
                        </div>
                        <div style="text-align: right;">
                            <span style="color: #43e97b; font-weight: 600; font-size: 1.1rem;">{row['Дослушиваемость']*100:.1f}%</span>
                            <span style="color: rgba(255,255,255,0.2); font-size: 0.65rem; margin-left: 0.4rem;">{row['Старты']:,}</span>
                        </div>
                    </div>
                    <div style="display: flex; gap: 1rem; margin-top: 0.2rem; font-size: 0.7rem; color: rgba(255,255,255,0.2);">
        ...
        with col2:
            st.markdown("""<div style="margin-bottom: 0.8rem;"><span style="font-size: 1.3rem; font-weight: 600; color: #f5576c;">⚠️ Зона риска</span><span style="font-size: 0.65rem; color: rgba(255,255,255,0.15); margin-left: 0.5rem; letter-spacing: 1.5px; text-transform: uppercase;">худшая дослушиваемость</span></div>""", unsafe_allow_html=True)
            for i, (_, row) in enumerate(danger_zone.iterrows()):
                st.markdown(f"""
                <div class="danger-zone">
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        <div>
                            <span style="font-size: 1.1rem;">{'⚠️' if i < 3 else '📌'}</span>
                            <span style="color: white; font-weight: 500; margin-left: 0.4rem; font-size: 0.9rem;">{row['Короткое название']}</span>
                        </div>
                        <div style="text-align: right;">
                            <span style="color: #f5576c; font-weight: 600; font-size: 1.1rem;">{row['Дослушиваемость']*100:.1f}%</span>
                            <span style="color: rgba(255,255,255,0.2); font-size: 0.65rem; margin-left: 0.4rem;">{row['Старты']:,}</span>
                        </div>
                    </div>
                    <div style="display: flex; gap: 1rem; margin-top: 0.2rem; font-size: 0.7rem; color: rgba(255,255,255,0.2);">
    ...
    with col1:
        ...
        fig2.add_trace(go.Bar(
            ...
            marker=dict(color=top_rsi['RSI'], colorscale='Viridis', showscale=True, colorbar=dict(title='RSI', titlefont=dict(color='rgba(255,255,255,0.3)'), tickfont=dict(color='rgba(255,255,255,0.3)'))),
            text=top_rsi['RSI'].round(1), textposition='outside', textfont=dict(color='rgba(255,255,255,0.4)', size=10),
        ...
        fig2.update_layout(
            ...
            paper_bgcolor='rgba(0,0,0,0)',
            height=420,
            xaxis=dict(tickangle=-20, tickfont=dict(color='rgba(255,255,255,0.3)', size=9), gridcolor='rgba(255,255,255,0.03)'),
            yaxis=dict(title='RSI', titlefont=dict(color='rgba(255,255,255,0.3)'), tickfont=dict(color='rgba(255,255,255,0.3)'), gridcolor='rgba(255,255,255,0.03)')
        )
        st.plotly_chart(fig2, use_container_width=True)
    with col2:
        st.markdown("""
        <div style="background: rgba(255,255,255,0.02); backdrop-filter: blur(10px); border: 1px solid rgba(255,255,255,0.04); border-radius: 14px; padding: 1.2rem;">
            <div style="color: rgba(255,255,255,0.2); font-size: 0.65rem; letter-spacing: 1.5px; text-transform: uppercase; margin-bottom: 0.8rem;">⭐ Топ RSI</div>
        """, unsafe_allow_html=True)
        if len(top_rsi) > 0:
            for i, (idx, row) in enumerate(top_rsi.head(5).iterrows()):
                medal = ['🥇', '🥈', '🥉', '4️⃣', '5️⃣'][i]
                st.markdown(f"""
                <div style="display: flex; align-items: center; justify-content: space-between; padding: 0.3rem 0; border-bottom: 1px solid rgba(255,255,255,0.02);">
                    <div>
                        <span style="color: rgba(255,255,255,0.2); font-size: 0.8rem;">{medal}</span>
                        <span style="color: rgba(255,255,255,0.6); font-size: 0.85rem; margin-left: 0.3rem;">{row['Короткое название']}</span>
                    </div>
                    <span style="color: #4facfe; font-weight: 600; font-size: 0.85rem;">{row['RSI']:.1f}</span>
                </div>
                """, unsafe_allow_html=True)
    ...
    with col1:
        st.markdown('<div style="color: #4facfe; font-weight: 600; font-size: 0.95rem; padding: 0.3rem 0;">📊 По стартам</div>', unsafe_allow_html=True)
        ...
        fig3 = go.Figure(data=[go.Bar(
            ...
            text=top_genre_starts['Старты'], textposition='outside', textfont=dict(color='rgba(255,255,255,0.3)', size=8)
        )])
        fig3.update_layout(
            template='plotly_dark',
            plot_bgcolor='rgba(0,0,0,0)',
            paper_bgcolor='rgba(0,0,0,0)',
            height=260,
            xaxis=dict(tickangle=-15, tickfont=dict(color='rgba(255,255,255,0.2)', size=8)),
            yaxis=dict(title='Старты', titlefont=dict(color='rgba(255,255,255,0.2)'), tickfont=dict(color='rgba(255,255,255,0.2)'), gridcolor='rgba(255,255,255,0.03)'),
            showlegend=False,
            margin=dict(l=10, r=10, t=10, b=30)
        )
        st.plotly_chart(fig3, use_container_width=True)
    with col2:
        st.markdown('<div style="color: #f5576c; font-weight: 600; font-size: 0.95rem; padding: 0.3rem 0;">🎧 По стримам</div>', unsafe_allow_html=True)
        ...
        fig4 = go.Figure(data=[go.Bar(
            ...
            text=top_genre_streams['Стримы'], textposition='outside', textfont=dict(color='rgba(255,255,255,0.3)', size=8)
        )])
        fig4.update_layout(
            template='plotly_dark',
            plot_bgcolor='rgba(0,0,0,0)',
            ...
            height=260,
            xaxis=dict(tickangle=-15, tickfont=dict(color='rgba(255,255,255,0.2)', size=8)),
            yaxis=dict(title='Стримы', titlefont=dict(color='rgba(255,255,255,0.2)'), tickfont=dict(color='rgba(255,255,255,0.2)'), gridcolor='rgba(255,255,255,0.03)'),
            showlegend=False,
            margin=dict(l=10, r=10, t=10, b=30)
        )
        st.plotly_chart(fig4, use_container_width=True)
    with col3:
        st.markdown('<div style="color: #43e97b; font-weight: 600; font-size: 0.95rem; padding: 0.3rem 0;">📈 По дослушиваемости</div>', unsafe_allow_html=True)
        ...
        fig5 = go.Figure(data=[go.Bar(
            ...
            text=top_genre_completion['Дослушиваемость'] * 100, texttemplate='%{text:.1f}%', textposition='outside', textfont=dict(color='rgba(255,255,255,0.3)', size=8)
        )])
        fig5.update_layout(
            template='plotly_dark',
            plot_bgcolor='rgba(0,0,0,0)',
            ...
            height=260,
            xaxis=dict(tickangle=-15, tickfont=dict(color='rgba(255,255,255,0.2)', size=8)),
            yaxis=dict(title='Дослушиваемость %', titlefont=dict(color='rgba(255,255,255,0.2)'), tickfont=dict(color='rgba(255,255,255,0.2)'), gridcolor='rgba(255,255,255,0.03)'),
            showlegend=False,
            margin=dict(l=10, r=10, t=10, b=30)
        )
    ...
    show_hint("💡", "Зачем смотреть", "Показывает, в какие дни недели у вас пик прослушиваний. Планируйте релизы на эти дни.")
    ...
    with col1:
        ...
        fig6 = go.Figure(data=[go.Bar(
            ...
            marker=dict(color=['#4facfe', '#43e97b', '#f093fb', '#f5576c', '#fa709a', '#f6d365', '#a8edea'], line=dict(color='rgba(255,255,255,0.05)', width=1)),
            text=weekday_stats['Старты'], textposition='outside', textfont=dict(color='rgba(255,255,255,0.3)', size=9)
        )])
        fig6.update_layout(
            template='plotly_dark',
            plot_bgcolor='rgba(0,0,0,0)',
            paper_bgcolor='rgba(0,0,0,0)',
            height=280,
            xaxis=dict(title='День недели', titlefont=dict(color='rgba(255,255,255,0.2)'), tickfont=dict(color='rgba(255,255,255,0.2)')),
            yaxis=dict(title='Старты', titlefont=dict(color='rgba(255,255,255,0.2)'), tickfont=dict(color='rgba(255,255,255,0.2)'), gridcolor='rgba(255,255,255,0.03)'),
            showlegend=False
        )
    ...
    with col2:
        ...
        fig7 = go.Figure(data=[go.Bar(
            ...
            marker=dict(color=['#4facfe', '#43e97b', '#f093fb', '#f5576c', '#fa709a', '#f6d365', '#a8edea'], line=dict(color='rgba(255,255,255,0.05)', width=1)),
            text=weekday_stats['Стримы'], textposition='outside', textfont=dict(color='rgba(255,255,255,0.3)', size=9)
        )])
        fig7.update_layout(
            template='plotly_dark',
            plot_bgcolor='rgba(0,0,0,0)',
    ...
    with st.expander("ℹ️ Информация о данных", expanded=False):
        ...
        with col1:
            st.markdown(f"""
            <div style="background: rgba(255,255,255,0.02); backdrop-filter: blur(8px); border: 1px solid rgba(255,255,255,0.04); border-radius: 12px; padding: 1rem;">
                <div style="color: rgba(255,255,255,0.15); font-size: 0.6rem; letter-spacing: 1.5px; text-transform: uppercase;">📅 Период</div>
                <div style="color: rgba(255,255,255,0.6); font-size: 1rem; font-weight: 500; margin-top: 0.2rem;">{filtered_data['Дата прослушивания'].min().date()} — {filtered_data['Дата прослушивания'].max().date()}</div>
            </div>
            <br>
            <div style="background: rgba(255,255,255,0.02); backdrop-filter: blur(8px); border: 1px solid rgba(255,255,255,0.04); border-radius: 12px; padding: 1rem;">
                <div style="color: rgba(255,255,255,0.15); font-size: 0.6rem; letter-spacing: 1.5px; text-transform: uppercase;">📊 Всего записей</div>
                <div style="color: rgba(255,255,255,0.6); font-size: 1rem; font-weight: 500; margin-top: 0.2rem;">{len(filtered_data):,}</div>
            </div>
            """, unsafe_allow_html=True)
        with col2:
            st.markdown(f"""
            <div style="background: rgba(255,255,255,0.02); backdrop-filter: blur(8px); border: 1px solid rgba(255,255,255,0.04); border-radius: 12px; padding: 1rem;">
                <div style="color: rgba(255,255,255,0.15); font-size: 0.6rem; letter-spacing: 1.5px; text-transform: uppercase;">📝 Уникальных выпусков</div>
                <div style="color: rgba(255,255,255,0.6); font-size: 1rem; font-weight: 500; margin-top: 0.2rem;">{filtered_data['Выпуск'].nunique()}</div>
            </div>
            <br>
            <div style="background: rgba(255,255,255,0.02); backdrop-filter: blur(8px); border: 1px solid rgba(255,255,255,0.04); border-radius: 12px; padding: 1rem;">
                <div style="color: rgba(255,255,255,0.15); font-size: 0.6rem; letter-spacing: 1.5px; text-transform: uppercase;">🎭 Жанры</div>
                <div style="color: rgba(255,255,255,0.4); font-size: 0.85rem; margin-top: 0.2rem;">{', '.join(filtered_data['Жанр'].unique())}</div>
            </div>
            """, unsafe_allow_html=True)
        with col3:
            st.markdown(f"""
            <div style="background: rgba(255,255,255,0.02); backdrop-filter: blur(8px); border: 1px solid rgba(255,255,255,0.04); border-radius: 12px; padding: 1rem;">
                <div style="color: rgba(255,255,255,0.15); font-size: 0.6rem; letter-spacing: 1.5px; text-transform: uppercase;">📂 Форматы</div>
                <div style="color: rgba(255,255,255,0.4); font-size: 0.85rem; margin-top: 0.2rem;">{', '.join(filtered_data['Формат'].unique())}</div>
            </div>
            <br>
            <div style="background: rgba(255,255,255,0.02); backdrop-filter: blur(8px); border: 1px solid rgba(255,255,255,0.04); border-radius: 12px; padding: 1rem;">
                <div style="color: rgba(255,255,255,0.15); font-size: 0.6rem; letter-spacing: 1.5px; text-transform: uppercase;">⭐ Средний RSI</div>
                <div style="color: rgba(255,255,255,0.6); font-size: 1rem; font-weight: 500; margin-top: 0.2rem;">{episode_summary['RSI'].mean():.1f}</div>
            </div>
            """, unsafe_allow_html=True)
...
# ============================================
    ...
    st.markdown('<div style="font-size: 2.2rem; font-weight: 600; color: #ffffff; margin-bottom: 0.3rem;">📋 Детальный анализ выпуска</div>', unsafe_allow_html=True)
    ...
    if all_data.empty:
        ...
        if episode_data.empty:
            ...
            with col1:
                st.markdown(f"""
                <div style="background: rgba(255,255,255,0.02); backdrop-filter: blur(8px); border: 1px solid rgba(255,255,255,0.04); border-radius: 12px; padding: 0.7rem; text-align: center;">
                    <div style="color: rgba(255,255,255,0.15); font-size: 0.55rem; letter-spacing: 1.5px; text-transform: uppercase;">📂 Формат</div>
                    <div style="color: rgba(255,255,255,0.6); font-weight: 500; font-size: 0.9rem; margin-top: 0.1rem;">{info['Формат']}</div>
                </div>
                """, unsafe_allow_html=True)
            with col2:
                st.markdown(f"""
                <div style="background: rgba(255,255,255,0.02); backdrop-filter: blur(8px); border: 1px solid rgba(255,255,255,0.04); border-radius: 12px; padding: 0.7rem; text-align: center;">
                    <div style="color: rgba(255,255,255,0.15); font-size: 0.55rem; letter-spacing: 1.5px; text-transform: uppercase;">🎭 Жанр</div>
                    <div style="color: rgba(255,255,255,0.6); font-weight: 500; font-size: 0.9rem; margin-top: 0.1rem;">{info['Жанр']}</div>
                </div>
                """, unsafe_allow_html=True)
            with col3:
                st.markdown(f"""
                <div style="background: rgba(255,255,255,0.02); backdrop-filter: blur(8px); border: 1px solid rgba(255,255,255,0.04); border-radius: 12px; padding: 0.7rem; text-align: center;">
                    <div style="color: rgba(255,255,255,0.15); font-size: 0.55rem; letter-spacing: 1.5px; text-transform: uppercase;">📅 Первая дата</div>
                    <div style="color: rgba(255,255,255,0.6); font-weight: 500; font-size: 0.9rem; margin-top: 0.1rem;">{release_date.date()}</div>
                </div>
                """, unsafe_allow_html=True)
            with col4:
                st.markdown(f"""
                <div style="background: rgba(255,255,255,0.02); backdrop-filter: blur(8px); border: 1px solid rgba(255,255,255,0.04); border-radius: 12px; padding: 0.7rem; text-align: center;">
                    <div style="color: rgba(255,255,255,0.15); font-size: 0.55rem; letter-spacing: 1.5px; text-transform: uppercase;">📆 Дней в выборке</div>
                    <div style="color: rgba(255,255,255,0.6); font-weight: 500; font-size: 0.9rem; margin-top: 0.1rem;">{days_active + 1}</div>
                </div>
                """, unsafe_allow_html=True)
            with col5:
                st.markdown(f"""
                <div style="background: rgba(255,255,255,0.02); backdrop-filter: blur(8px); border: 1px solid rgba(255,255,255,0.04); border-radius: 12px; padding: 0.7rem; text-align: center;">
                    <div style="color: rgba(255,255,255,0.15); font-size: 0.55rem; letter-spacing: 1.5px; text-transform: uppercase;">⏱️ Длительность</div>
                    <div style="color: rgba(255,255,255,0.6); font-weight: 500; font-size: 0.9rem; margin-top: 0.1rem;">{info['Длительность']}</div>
                </div>
                """, unsafe_allow_html=True)
            ...
            fig = go.Figure()
            fig.add_trace(go.Scatter(x=daily_data['Дата прослушивания'], y=daily_data['Старты'], name='Старты', line=dict(color='#4facfe', width=3), fill='tozeroy', fillcolor='rgba(79, 172, 254, 0.08)'))
            fig.add_trace(go.Scatter(x=daily_data['Дата прослушивания'], y=daily_data['Стримы'], name='Стримы', line=dict(color='#f5576c', width=3), fill='tozeroy', fillcolor='rgba(245, 87, 108, 0.08)'))
            fig.add_shape(type="line", x0=release_date, y0=0, x1=release_date, y1=1, yref="paper", line=dict(color="#f093fb", width=2, dash="dash"))
            fig.add_annotation(x=release_date, y=0.98, yref="paper", text="📅 Релиз", showarrow=False, font=dict(color="#f093fb", size=10), textangle=-90)
            ...
            if life_curve is not None and not life_curve.empty:
                ...
                fig_life.add_trace(go.Scatter(
                    ...
                    mode='lines+markers',
                    marker=dict(size=7, color='white', line=dict(color='#f5576c', width=1.5)),
                    fill='tozeroy',
                ...
                try:
                    ...
                    if idx_50 is not None:
                        ...
                        fig_life.add_annotation(x=day_50, y=50, text=f"⚡ 50% на день {int(day_50)}", showarrow=True, arrowhead=2, ax=20, ay=-30, font=dict(color='#f6d365', size=9), arrowcolor='#f6d365')
                    ...
                    if idx_90 is not None:
                        ...
                        fig_life.add_annotation(x=day_90, y=90, text=f"🎯 90% на день {int(day_90)}", showarrow=True, arrowhead=2, ax=20, ay=30, font=dict(color='#43e97b', size=9), arrowcolor='#43e97b')
                except:
                    pass
                ...
                fig_life.update_layout(
                    ...
                    hovermode='x unified',
                    legend=dict(orientation='h', yanchor='bottom', y=1.02, xanchor='right', x=1, font=dict(color='rgba(255,255,255,0.3)')),
                    xaxis=dict(title='День от релиза', titlefont=dict(color='rgba(255,255,255,0.2)'), tickfont=dict(color='rgba(255,255,255,0.2)'), gridcolor='rgba(255,255,255,0.03)'),
                    yaxis=dict(title='% от всех стримов', titlefont=dict(color='#f5576c'), tickfont=dict(color='rgba(255,255,255,0.2)'), gridcolor='rgba(255,255,255,0.03)', range=[0, 105]),
                    yaxis2=dict(title='Стримы (абс.)', titlefont=dict(color='#f093fb'), tickfont=dict(color='rgba(255,255,255,0.2)'), overlaying='y', side='right', showgrid=False)
                )
                ...
                with col3:
                    ...
                    st.markdown(f"""
                    <div style="background: rgba(255,255,255,0.02); backdrop-filter: blur(8px); border: 1px solid {color}33; border-radius: 10px; padding: 0.6rem; text-align: center;">
            ...
            if funnel_data['has_data']:
                ...
                fig_funnel.add_trace(go.Funnel(
                    ...
                    marker=dict(color=colors, line=dict(width=2, color='rgba(255,255,255,0.05)')),
                ...
                fig_funnel.update_layout(
                    ...
                    height=380,
                    showlegend=False,
                    xaxis=dict(title='Количество слушателей', titlefont=dict(color='rgba(255,255,255,0.2)'), tickfont=dict(color='rgba(255,255,255,0.2)'), gridcolor='rgba(255,255,255,0.03)')
                )
                ...
                with col1:
                    ...
                    <div style="background: rgba(255,255,255,0.02); backdrop-filter: blur(8px); border: 1px solid {color}33; border-radius: 10px; padding: 0.6rem; text-align: center;">
                        <div style="color: {color}; font-weight: 600; font-size: 0.9rem;">{status}</div>
                        <div style="color: rgba(255,255,255,0.2); font-size: 0.7rem; margin-top: 0.1rem;">Потеря: {drop_1_pct:.0f}% ({drop_1:,.0f} чел.)</div>
                    </div>
                    """, unsafe_allow_html=True)
                with col2:
                    ...
                    <div style="background: rgba(255,255,255,0.02); backdrop-filter: blur(8px); border: 1px solid {color}33; border-radius: 10px; padding: 0.6rem; text-align: center;">
                        <div style="color: {color}; font-weight: 600; font-size: 0.9rem;">{status}</div>
                        <div style="color: rgba(255,255,255,0.2); font-size: 0.7rem; margin-top: 0.1rem;">Потеря: {drop_2_pct:.0f}% ({drop_2:,.0f} чел.)</div>
                    </div>
                    """, unsafe_allow_html=True)
                with col3:
                    ...
                    st.markdown(f"""
                    <div style="background: rgba(255,255,255,0.02); backdrop-filter: blur(8px); border: 1px solid {color}33; border-radius: 10px; padding: 0.6rem; text-align: center;">
                        <div style="color: {color}; font-weight: 600; font-size: 0.9rem;">{verdict}</div>
                        <div style="color: rgba(255,255,255,0.2); font-size: 0.7rem; margin-top: 0.1rem;">Дослушивают: {funnel_data["completion"]*100:.0f}% ({funnel_data["stage_3"]:,.0f} чел.)</div>
                    </div>
                    """, unsafe_allow_html=True)
            else:
...
# ============================================
    ...
    st.markdown('<div style="font-size: 2.2rem; font-weight: 600; color: #ffffff; margin-bottom: 0.3rem;">🔄 Сравнение двух выпусков</div>', unsafe_allow_html=True)
    ...
    if ep1 == ep2:
        ...
        if data1.empty or data2.empty:
            ...
            with col1:
                st.markdown(f"""
                <div style="background: rgba(79, 172, 254, 0.04); border: 1px solid rgba(79, 172, 254, 0.1); border-radius: 14px; padding: 0.3rem; margin-bottom: 0.8rem;">
                    <h3 style="color: #4facfe; text-align: center; margin: 0.3rem 0; font-weight: 500; font-size: 1.1rem;">{ep1_short}</h3>
                </div>
                """, unsafe_allow_html=True)
                subcol1, subcol2, subcol3, subcol4 = st.columns(4)
                with subcol1:
                    st.markdown(f"""
                    <div class="metric-card" style="padding: 0.6rem 0.3rem;">
                        <span class="metric-icon" style="font-size: 1.1rem;">🎬</span>
                        <div class="metric-value" style="font-size: 1.4rem;">{total_starts1:,}</div>
                        <div class="metric-label" style="font-size: 0.55rem;">Старты</div>
                    </div>
                    """, unsafe_allow_html=True)
                with subcol2:
                    st.markdown(f"""
                    <div class="metric-card" style="padding: 0.6rem 0.3rem;">
                        <span class="metric-icon" style="font-size: 1.1rem;">🎧</span>
                        <div class="metric-value" style="font-size: 1.4rem;">{total_streams1:,}</div>
                        <div class="metric-label" style="font-size: 0.55rem;">Стримы</div>
                    </div>
                    """, unsafe_allow_html=True)
                with subcol3:
                    st.markdown(f"""
                    <div class="metric-card" style="padding: 0.6rem 0.3rem;">
                        <span class="metric-icon" style="font-size: 1.1rem;">📈</span>
                        <div class="metric-value" style="font-size: 1.4rem;">{conv1:.1f}%</div>
                        <div class="metric-label" style="font-size: 0.55rem;">Конверсия</div>
                    </div>
                    """, unsafe_allow_html=True)
                with subcol4:
                    st.markdown(f"""
                    <div class="metric-card" style="padding: 0.6rem 0.3rem;">
                        <span class="metric-icon" style="font-size: 1.1rem;">⭐</span>
                        <div class="metric-value" style="font-size: 1.4rem;">{rsi1:.1f}</div>
                        <div class="metric-label" style="font-size: 0.55rem;">RSI</div>
                    </div>
                    """, unsafe_allow_html=True)
            
            with col2:
                st.markdown(f"""
                <div style="background: rgba(245, 87, 108, 0.04); border: 1px solid rgba(245, 87, 108, 0.1); border-radius: 14px; padding: 0.3rem; margin-bottom: 0.8rem;">
                    <h3 style="color: #f5576c; text-align: center; margin: 0.3rem 0; font-weight: 500; font-size: 1.1rem;">{ep2_short}</h3>
                </div>
                """, unsafe_allow_html=True)
                subcol1, subcol2, subcol3, subcol4 = st.columns(4)
                with subcol1:
                    ...
                    <div class="metric-card" style="padding: 0.6rem 0.3rem;">
                        <span class="metric-icon" style="font-size: 1.1rem;">🎬</span>
                        <div class="metric-value" style="font-size: 1.4rem;">{total_starts2:,}</div>
                        <div class="metric-label" style="font-size: 0.55rem;">Старты</div>
                    </div>
                    """, unsafe_allow_html=True)
                with subcol2:
                    ...
                    <div class="metric-card" style="padding: 0.6rem 0.3rem;">
                        <span class="metric-icon" style="font-size: 1.1rem;">🎧</span>
                        <div class="metric-value" style="font-size: 1.4rem;">{total_streams2:,}</div>
                        <div class="metric-label" style="font-size: 0.55rem;">Стримы</div>
                    </div>
                    """, unsafe_allow_html=True)
                with subcol3:
                    ...
                    <div class="metric-card" style="padding: 0.6rem 0.3rem;">
                        <span class="metric-icon" style="font-size: 1.1rem;">📈</span>
                        <div class="metric-value" style="font-size: 1.4rem;">{conv2:.1f}%</div>
                        <div class="metric-label" style="font-size: 0.55rem;">Конверсия</div>
                    </div>
                    """, unsafe_allow_html=True)
                with subcol4:
                    ...
                    <div class="metric-card" style="padding: 0.6rem 0.3rem;">
                        <span class="metric-icon" style="font-size: 1.1rem;">⭐</span>
                        <div class="metric-value" style="font-size: 1.4rem;">{rsi2:.1f}</div>
                        <div class="metric-label" style="font-size: 0.55rem;">RSI</div>
                    </div>
                    """, unsafe_allow_html=True)
            ...
            st.markdown('<div style="color: #4facfe; font-weight: 500; font-size: 0.9rem; padding: 0.3rem 0;">🎬 Сравнение стартов</div>', unsafe_allow_html=True)
            fig_starts = go.Figure()
            fig_starts.add_trace(go.Scatter(x=daily1['День от релиза'], y=daily1['Старты'], name=f'{ep1_short}', line=dict(color='#4facfe', width=3), mode='lines+markers', marker=dict(size=7, color='white', line=dict(color='#4facfe', width=1.5)), fill='tozeroy', fillcolor='rgba(79, 172, 254, 0.08)'))
            fig_starts.add_trace(go.Scatter(x=daily2['День от релиза'], y=daily2['Старты'], name=f'{ep2_short}', line=dict(color='#f5576c', width=3), mode='lines+markers', marker=dict(size=7, color='white', line=dict(color='#f5576c', width=1.5)), fill='tozeroy', fillcolor='rgba(245, 87, 108, 0.08)'))
            fig_starts.update_layout(
                ...
                legend=dict(orientation='h', yanchor='bottom', y=1.02, xanchor='right', x=1, font=dict(color='rgba(255,255,255,0.3)')),
                xaxis=dict(title='День от релиза', titlefont=dict(color='rgba(255,255,255,0.2)'), tickfont=dict(color='rgba(255,255,255,0.2)'), gridcolor='rgba(255,255,255,0.03)'),
                yaxis=dict(title='Старты', titlefont=dict(color='#4facfe'), tickfont=dict(color='rgba(255,255,255,0.2)'), gridcolor='rgba(255,255,255,0.03)')
            )
            st.plotly_chart(fig_starts, use_container_width=True)
            
            st.markdown('<div style="color: #f5576c; font-weight: 500; font-size: 0.9rem; padding: 0.3rem 0;">🎧 Сравнение стримов</div>', unsafe_allow_html=True)
            fig_streams = go.Figure()
            fig_streams.add_trace(go.Scatter(x=daily1['День от релиза'], y=daily1['Стримы'], name=f'{ep1_short}', line=dict(color='#43e97b', width=3), mode='lines+markers', marker=dict(size=7, color='white', line=dict(color='#43e97b', width=1.5)), fill='tozeroy', fillcolor='rgba(67, 233, 123, 0.08)'))
            fig_streams.add_trace(go.Scatter(x=daily2['День от релиза'], y=daily2['Стримы'], name=f'{ep2_short}', line=dict(color='#f093fb', width=3), mode='lines+markers', marker=dict(size=7, color='white', line=dict(color='#f093fb', width=1.5)), fill='tozeroy', fillcolor='rgba(240, 147, 251, 0.08)'))
            fig_streams.update_layout(
                ...
                legend=dict(orientation='h', yanchor='bottom', y=1.02, xanchor='right', x=1, font=dict(color='rgba(255,255,255,0.3)')),
                xaxis=dict(title='День от релиза', titlefont=dict(color='rgba(255,255,255,0.2)'), tickfont=dict(color='rgba(255,255,255,0.2)'), gridcolor='rgba(255,255,255,0.03)'),
                yaxis=dict(title='Стримы', titlefont=dict(color='#f5576c'), tickfont=dict(color='rgba(255,255,255,0.2)'), gridcolor='rgba(255,255,255,0.03)')
            )
            ...
            if life_curve1 is not None and life_curve2 is not None and not life_curve1.empty and not life_curve2.empty:
                ...
                fig_compare_life.add_trace(go.Scatter(
                    ...
                    mode='lines+markers',
                    marker=dict(size=7, color='white', line=dict(color='#4facfe', width=1.5)),
                    fill='tozeroy',
                ...
                for y_val, color, label in [(50, '#f6d365', '50%'), (90, '#43e97b', '90%')]:
                    fig_compare_life.add_hline(y=y_val, line_dash="dash", line_color=color, line_width=1.5, annotation_text=label, annotation_font=dict(color=color, size=8))
                ...
                fig_compare_life.update_layout(
                    ...
                    legend=dict(orientation='h', yanchor='bottom', y=1.02, xanchor='right', x=1, font=dict(color='rgba(255,255,255,0.3)')),
                    xaxis=dict(title='День от релиза', titlefont=dict(color='rgba(255,255,255,0.2)'), tickfont=dict(color='rgba(255,255,255,0.2)'), gridcolor='rgba(255,255,255,0.03)'),
                    yaxis=dict(title='% от всех стримов', titlefont=dict(color='rgba(255,255,255,0.2)'), tickfont=dict(color='rgba(255,255,255,0.2)'), gridcolor='rgba(255,255,255,0.03)', range=[0, 105])
                )
                ...
                with col1:
                    ...
                    if days1_50 and days2_50:
                        ...
                        st.markdown(f"""
                        <div style="background: rgba(255,255,255,0.02); backdrop-filter: blur(8px); border: 1px solid rgba(79, 172, 254, 0.1); border-radius: 10px; padding: 0.8rem; text-align: center;">
                            <div style="color: #4facfe; font-weight: 500; font-size: 0.85rem;">⏱️ Дней до 50% {faster}</div>
                            <div style="color: rgba(255,255,255,0.4); font-size: 0.8rem; margin-top: 0.2rem;">
                                {ep1_short}: <span style="color: #4facfe; font-weight: 500;">{days1_50} дн.</span><br>
                                {ep2_short}: <span style="color: #f5576c; font-weight: 500;">{days2_50} дн.</span>
                            </div>
                        </div>
                        """, unsafe_allow_html=True)
                with col2:
                    ...
                    if days1_90 and days2_90:
                        ...
                        st.markdown(f"""
                        <div style="background: rgba(255,255,255,0.02); backdrop-filter: blur(8px); border: 1px solid rgba(67, 233, 123, 0.1); border-radius: 10px; padding: 0.8rem; text-align: center;">
                            <div style="color: #43e97b; font-weight: 500; font-size: 0.85rem;">⏱️ Дней до 90% {longer}</div>
                            <div style="color: rgba(255,255,255,0.4); font-size: 0.8rem; margin-top: 0.2rem;">
                                {ep1_short}: <span style="color: #4facfe; font-weight: 500;">{days1_90} дн.</span><br>
                                {ep2_short}: <span style="color: #f5576c; font-weight: 500;">{days2_90} дн.</span>
                            </div>
                        </div>
                        """, unsafe_allow_html=True)
                with col3:
                    if len(life_curve1) >= 3 and len(life_curve2) >= 3:
                        ...
                        st.markdown(f"""
                        <div style="background: rgba(255,255,255,0.02); backdrop-filter: blur(8px); border: 1px solid rgba(246, 211, 101, 0.1); border-radius: 10px; padding: 0.8rem; text-align: center;">
                            <div style="color: #f6d365; font-weight: 500; font-size: 0.85rem;">🚀 Скорость старта {faster_start}</div>
                            <div style="color: rgba(255,255,255,0.4); font-size: 0.8rem; margin-top: 0.2rem;">
                                {ep1_short}: <span style="color: #4facfe; font-weight: 500;">{slope1:.1f}%/день</span><br>
                                {ep2_short}: <span style="color: #f5576c; font-weight: 500;">{slope2:.1f}%/день</span>
                            </div>
                        </div>
                        """, unsafe_allow_html=True)
                ...
                with col1:
                    st.markdown(f"""
                    <div style="background: rgba(255,255,255,0.02); backdrop-filter: blur(8px); border: 1px solid rgba(79, 172, 254, 0.1); border-radius: 10px; padding: 0.6rem; text-align: center;">
                        <div style="color: #4facfe; font-weight: 500; font-size: 0.85rem;">⭐ {ep1_short}</div>
                        <div style="color: rgba(255,255,255,0.2); font-size: 0.7rem;">Скорость: {slope1:.1f}%/день</div>
                    </div>
                    """, unsafe_allow_html=True)
                with col2:
                    st.markdown(f"""
                    <div style="background: rgba(255,255,255,0.02); backdrop-filter: blur(8px); border: 1px solid rgba(245, 87, 108, 0.1); border-radius: 10px; padding: 0.6rem; text-align: center;">
                        <div style="color: #f5576c; font-weight: 500; font-size: 0.85rem;">⭐ {ep2_short}</div>
                        <div style="color: rgba(255,255,255,0.2); font-size: 0.7rem;">Скорость: {slope2:.1f}%/день</div>
                    </div>
                    """, unsafe_allow_html=True)
                with col3:
                    if days1_50 and days2_50 and days1_90 and days2_90:
                        if days1_50 < days2_50 and days1_90 < days2_90:
                            winner, color, detail = ep1_short, "#43e97b", "быстрее набирает, но быстрее выдыхается"
                        elif days1_50 > days2_50 and days1_90 > days2_90:
                            winner, color, detail = ep2_short, "#43e97b", "быстрее набирает, но быстрее выдыхается"
                        elif days1_50 < days2_50 and days1_90 > days2_90:
                        ...
                        st.markdown(f"""
                        <div style="background: rgba(255,255,255,0.02); backdrop-filter: blur(8px); border: 1px solid {color}44; border-radius: 10px; padding: 0.6rem; text-align: center;">
                            <div style="color: {color}; font-weight: 500; font-size: 0.85rem;">🏆 Победитель</div>
                            <div style="color: {color}; font-size: 1.1rem; font-weight: 600;">{winner}</div>
                            <div style="color: rgba(255,255,255,0.2); font-size: 0.7rem;">{detail}</div>
                        </div>
                        """, unsafe_allow_html=True)
            ...
            if funnel1['has_data'] and funnel2['has_data']:
                ...
                with col1:
                    st.markdown(f"""
                    <div style="background: rgba(79, 172, 254, 0.04); border: 1px solid rgba(79, 172, 254, 0.1); border-radius: 10px; padding: 0.2rem; text-align: center; margin-bottom: 0.3rem;">
                        <h4 style="color: #4facfe; margin: 0.2rem; font-weight: 500; font-size: 0.9rem;">{ep1_short}</h4>
                    </div>
                    """, unsafe_allow_html=True)
                    ...
                    fig_funnel1.add_trace(go.Funnel(
                        ...
                        textinfo="value+percent initial",
                        textposition="inside",
                        textfont=dict(color="white", size=10, family="-apple-system, sans-serif"),
                        marker=dict(color=['#4facfe', '#f6d365', '#43e97b'], line=dict(width=2, color='rgba(255,255,255,0.05)'))
                    ))
                ...
                with col2:
                    st.markdown(f"""
                    <div style="background: rgba(245, 87, 108, 0.04); border: 1px solid rgba(245, 87, 108, 0.1); border-radius: 10px; padding: 0.2rem; text-align: center; margin-bottom: 0.3rem;">
                        <h4 style="color: #f5576c; margin: 0.2rem; font-weight: 500; font-size: 0.9rem;">{ep2_short}</h4>
                    </div>
                    """, unsafe_allow_html=True)
                    ...
                    fig_funnel2.add_trace(go.Funnel(
                        ...
                        textposition="inside",
                        textfont=dict(color="white", size=10, family="-apple-system, sans-serif"),
                        marker=dict(color=['#f5576c', '#f6d365', '#43e97b'], line=dict(width=2, color='rgba(255,255,255,0.05)'))
                    ))
                ...
                with col1:
                    ...
                    st.markdown(f"""
                    <div style="background: rgba(255,255,255,0.02); backdrop-filter: blur(8px); border: 1px solid {color}44; border-radius: 10px; padding: 0.6rem; text-align: center;">
                        <div style="color: {color}; font-weight: 500; font-size: 0.8rem;">🏆 Победитель по удержанию</div>
                        <div style="color: {color}; font-weight: 600; font-size: 1rem;">{winner}</div>
                        <div style="color: rgba(255,255,255,0.2); font-size: 0.7rem;">{detail}</div>
                    </div>
                    """, unsafe_allow_html=True)
                with col2:
                    ...
                    else: better, color, detail = "Ничья", "#f6d365", "одинаковые потери"
                    st.markdown(f"""
                    <div style="background: rgba(255,255,255,0.02); backdrop-filter: blur(8px); border: 1px solid {color}44; border-radius: 10px; padding: 0.6rem; text-align: center;">
                        <div style="color: {color}; font-weight: 500; font-size: 0.8rem;">🎯 Лучший старт</div>
                        <div style="color: {color}; font-weight: 600; font-size: 1rem;">{better}</div>
                        <div style="color: rgba(255,255,255,0.2); font-size: 0.7rem;">{detail}</div>
                    </div>
                    """, unsafe_allow_html=True)
                with col3:
                    ...
                    st.markdown(f"""
                    <div style="background: rgba(255,255,255,0.02); backdrop-filter: blur(8px); border: 1px solid {color}44; border-radius: 10px; padding: 0.6rem; text-align: center;">
                        <div style="color: {color}; font-weight: 500; font-size: 0.8rem;">🏁 Лучшая концовка</div>
                        <div style="color: {color}; font-weight: 600; font-size: 1rem;">{better}</div>
                        <div style="color: rgba(255,255,255,0.2); font-size: 0.7rem;">{detail}</div>
                    </div>
                    """, unsafe_allow_html=True)
            ...
            with col1:
                st.markdown(f"""
                <div style="background: rgba(255,255,255,0.02); backdrop-filter: blur(8px); border: 1px solid rgba(79, 172, 254, 0.1); border-radius: 10px; padding: 0.6rem; text-align: center;">
                    <div style="color: #4facfe; font-weight: 500; font-size: 0.8rem;">⭐ RSI {ep1_short}</div>
                    <div style="font-size: 1.4rem; font-weight: 600; color: rgba(255,255,255,0.8);">{rsi1:.1f}</div>
                </div>
                """, unsafe_allow_html=True)
            with col2:
                st.markdown(f"""
                <div style="background: rgba(255,255,255,0.02); backdrop-filter: blur(8px); border: 1px solid rgba(245, 87, 108, 0.1); border-radius: 10px; padding: 0.6rem; text-align: center;">
                    <div style="color: #f5576c; font-weight: 500; font-size: 0.8rem;">⭐ RSI {ep2_short}</div>
                    <div style="font-size: 1.4rem; font-weight: 600; color: rgba(255,255,255,0.8);">{rsi2:.1f}</div>
                </div>
                """, unsafe_allow_html=True)
            with col3:
                if rsi1 > rsi2 * 1.05:
                    st.markdown(f"""
                    <div style="background: rgba(67, 233, 123, 0.04); border: 1px solid #43e97b44; border-radius: 10px; padding: 0.6rem; text-align: center;">
                        <div style="color: #43e97b; font-weight: 500; font-size: 0.8rem;">🏆 Победитель</div>
                        <div style="color: #43e97b; font-size: 1.2rem; font-weight: 600;">{ep1_short}</div>
                        <div style="color: rgba(255,255,255,0.2); font-size: 0.7rem;">значительно лучше по RSI!</div>
                    </div>
                    """, unsafe_allow_html=True)
                elif rsi1 > rsi2:
                    st.markdown(f"""
                    <div style="background: rgba(79, 172, 254, 0.04); border: 1px solid #4facfe44; border-radius: 10px; padding: 0.6rem; text-align: center;">
                        <div style="color: #4facfe; font-weight: 500; font-size: 0.8rem;">🏆 Победитель</div>
                        <div style="color: #4facfe; font-size: 1.2rem; font-weight: 600;">{ep1_short}</div>
                        <div style="color: rgba(255,255,255,0.2); font-size: 0.7rem;">лучше по RSI!</div>
                    </div>
                    """, unsafe_allow_html=True)
                elif rsi2 > rsi1 * 1.05:
                    st.markdown(f"""
                    <div style="background: rgba(67, 233, 123, 0.04); border: 1px solid #43e97b44; border-radius: 10px; padding: 0.6rem; text-align: center;">
                        <div style="color: #43e97b; font-weight: 500; font-size: 0.8rem;">🏆 Победитель</div>
                        <div style="color: #43e97b; font-size: 1.2rem; font-weight: 600;">{ep2_short}</div>
                        <div style="color: rgba(255,255,255,0.2); font-size: 0.7rem;">значительно лучше по RSI!</div>
                    </div>
                    """, unsafe_allow_html=True)
                elif rsi2 > rsi1:
                    ...
                    <div style="background: rgba(79, 172, 254, 0.04); border: 1px solid #4facfe44; border-radius: 10px; padding: 0.6rem; text-align: center;">
                        <div style="color: #4facfe; font-weight: 500; font-size: 0.8rem;">🏆 Победитель</div>
                        <div style="color: #4facfe; font-size: 1.2rem; font-weight: 600;">{ep2_short}</div>
                        <div style="color: rgba(255,255,255,0.2); font-size: 0.7rem;">лучше по RSI!</div>
                    </div>
                    """, unsafe_allow_html=True)
                else:
                    st.markdown(f"""
                    <div style="background: rgba(246, 211, 101, 0.04); border: 1px solid #f6d36544; border-radius: 10px; padding: 0.6rem; text-align: center;">
                        <div style="color: #f6d365; font-weight: 500; font-size: 0.8rem;">🤝 Ничья</div>
                        <div style="color: rgba(255,255,255,0.2); font-size: 0.7rem;">выпуски примерно равны по RSI!</div>
                    </div>
                    """, unsafe_allow_html=True)
